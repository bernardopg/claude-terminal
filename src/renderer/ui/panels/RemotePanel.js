...
class RemotePanel extends BasePanel {
  ...
  setupHandlers() {
    ...
    const populateIfaceSelect = async () => {
      if (!ifaceSelect) return;
      try {
        const info = await this.api.remote.getServerInfo();
        const ifaces = info.networkInterfaces || [];
        const savedIp = this._settingsState.get().remoteSelectedIp || '';
    ...
    toggle.addEventListener('change', async () => {
      ...
      this._settingsState.setProp('remoteEnabled', enabled);
      this._saveSettings();
    ...
    if (portInput) {
      portInput.addEventListener('change', () => {
        ...
        this._settingsState.setProp('remotePort', port);
        this._saveSettings();
      });
    }
    ...
    if (ifaceSelect) {
      ifaceSelect.addEventListener('change', () => {
        ...
        this._settingsState.setProp('remoteSelectedIp', ip);
        this._saveSettings();
      });
    }
    ...
    if (toggleServerBtn) {
      toggleServerBtn.addEventListener('click', async () => {
        ...
        try {
          const info = await this.api.remote.getServerInfo();
          if (info.running) {
            await this.api.remote.stopServer();
          } else {
            await this.api.remote.startServer();
          }
          await this._refreshServerStatus();
        } finally {
  ...
  _stopPinPolling() {
    if (this._pinRefreshInterval) { clearInterval(this._pinRefreshInterval); this._pinRefreshInterval = null; }
  }
  ...
  async _refreshServerStatus() {
    ...
    try {
      ...
      if (info.running) {
        ...
        this._renderQrCode(info.address);
  ...
  async _refreshClientList() {
    ...
    try {
      ...
      list.innerHTML = clients.map(c => {
        const since = this._formatDuration(Date.now() - c.connectedAt);
        const device = this._parseUserAgent(c.userAgent);
        return `
      ...
      list.querySelectorAll('.rp-client-kick-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          btn.disabled = true;
          await this.api.remote.disconnectClient({ clientId: cid });
          this._refreshClientList();
        });
      });
    } catch (e) {}
...
function _ensureInstance(context) {
  if (!_instance) {
    ...
    _instance = new RemotePanel(null, {
      api: getApiProvider(),
      container: getContainer(),
      settingsState: context.settingsState,
...
module.exports = {
  RemotePanel,
  buildHtml: (settings) => {
    // buildHtml can be called before setupHandlers, create temp instance if needed
    const { getApiProvider, getContainer } = require('../../core');
    if (!_instance) {
      _instance = new RemotePanel(null, { api: getApiProvider(), container: getContainer() });
    }
    return _instance.buildHtml(settings);
  },
  setupHandlers: (context) => {
    _ensureInstance(context);
    // Update settings references in case they changed
